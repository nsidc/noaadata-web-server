...
from noaa_metrics.constants.paths import JSON_OUTPUT_FILEPATH, REPORT_OUTPUT_DIR, CDR_REPORT_OUTPUT_FILEPATH
...
def main():
    ...
    # breakpoint()
    summary_csv = df_to_csv(summary_df, 'NOAA CDR Requests for March 2023\n\n', CDR_REPORT_OUTPUT_FILEPATH)
    by_day_csv = df_to_csv(by_day_df, "\nTransfers by Day\n\n", CDR_REPORT_OUTPUT_FILEPATH)
    by_dataset_csv = df_to_csv(by_dataset_df, "\nTransfers by Dataset\n\n", CDR_REPORT_OUTPUT_FILEPATH)
    all_csv = df_to_csv(by_location_df, "\nTransfers by Domain\n\n", CDR_REPORT_OUTPUT_FILEPATH)
    
    email_full_report("/tmp/noaa-cdr-march-2023.csv", "noaa-cdr-march-2023.csv", "NOAA CDR Downloads March 2023")
